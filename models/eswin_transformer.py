...
class Convolutional_block(nn.Module):
    ...
    def __init__(self, embed_dim=96):
        ...
        self.b1 = nn.Sequential(nn.BatchNorm2d(3),
                                nn.Conv2d(3, 6, 3, 1, 1))
        self.b2 = nn.Sequential(nn.BatchNorm2d(6),
                                nn.Conv2d(6, 12, 3, 1, 1))
        self.b3 = nn.Sequential(nn.BatchNorm2d(12),
                                nn.Conv2d(12, 24, 3, 1, 1))
        self.b4 = nn.Sequential(nn.BatchNorm2d(24),
                      nn.Conv2d(24, 48, 3, 1, 1))
        self.b5 = nn.Sequential(nn.BatchNorm2d(48),
                                nn.Conv2d(48, 96, 3, 1, 1))
        self.b6 = nn.Conv2d(96, embed_dim, 1)
    def forward(self, x):
        x = F.relu(self.b1(x))
        x = F.relu(self.b2(x))
        x = F.relu(self.b3(x))
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=32, patch_size=1, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj = Convolutional_block(embed_dim)
...
class ESwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=32, patch_size=1, in_chans=3, num_classes=10,
